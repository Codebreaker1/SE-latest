...
(function($, document) {
  ...
  var u={
	  ...
	  // return new Date (string_date).getYear();
	  },
	  ...
	  createCanvas:function(width,height){
	    var canvas = document.createElement("canvas");
	  	...
	  	if (devicePixelRatio != 1) {
	  		var style = "width:" + width+5 + "px;height:" + height + "px"
	  ...
	  getXValues:function(elements, _x){
		  var x={}
  		  for(var i=0;i<elements.length;i++){
			  ...
			  if(i==0){
	 			 ...
	 			 //document.getElementById("ch").innerHTML = u.getTime();
	 			 x.min=u.getTime(data[0].date);
	 			  //document.getElementById("ms").innerHTML =x.min;
			  }else{
		  ...
		  if(_x.max){x.max=u.getTime(_x.max)
		  	//document.getElementById("ms").innerHTML =x.max;
		  }
		  if(_x.min){x.min=u.getTime(_x.min)}
		  x.margin= _x.margin;
		//document.getElementById("ch").innerHTML = x.margin;
		  return x;
	  },
	  ...
	  fDate:function(d){
	  		...
	  		//document.getElementById("ms").innerHTML=d.getYear();
  		return d.getYear()-100 ;
  	
	  },
	  deltaDays: function(min, max){
		  var oneDay = 24*60*60*1000; // hours*minutes*seconds*milliseconds
		// document.getElementById("ms").innerHTML =Math.round(Math.abs((min - max)/(oneDay)));
  ...
  pista.defaults = {
	  ...
	  labels:{
		  lineWidth: 0.3,
		  fontSize: 11,
		  x:{number: 7, show:true, color:"#858585", grid:true },
